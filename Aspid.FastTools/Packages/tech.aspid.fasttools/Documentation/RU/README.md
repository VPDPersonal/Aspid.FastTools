...
- Потерянный тип можно починить на месте: в предупреждении есть кнопка **Edit Type**, переписывающая сохранённые Assembly / Namespace / Class прямо в YAML ассета (на ассетах-ScriptableObject — единственных, где Unity сохраняет потерянные ссылки).
