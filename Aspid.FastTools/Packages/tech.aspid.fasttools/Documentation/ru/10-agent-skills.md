...
Обе команды берут скиллы из ветки `main`, где они могут описывать API, которого ещё нет в вашей версии пакета. Чтобы скиллы совпадали с установленным пакетом, ставьте их с тега его релиза; `npx skills update` тогда остаётся на этом теге, а после обновления пакета снова выполните `add` с новым тегом:

```bash
npx skills add VPDPersonal/Aspid.FastTools#v<version>
```

Релизы до `1.0.0-rc.8` включительно не содержат этих скиллов; с ними ставьте скиллы из `main`.
