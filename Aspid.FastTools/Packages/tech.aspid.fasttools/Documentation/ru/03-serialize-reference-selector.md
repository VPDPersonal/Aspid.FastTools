...
Для ассета на диске Fix переписывает сохранённую запись типа и переимпортирует ассет; обычного Undo у такой записи в файл нет. В открытой сохранённой сцене, Prefab Mode и ассете с несохранёнными изменениями восстановление применяется к объекту в памяти, и эти изменения не теряются — после проверки результата сохраните сцену, префаб или ассет. Пока префаб открыт в Prefab Mode, Fix на его ассете в окне Project отклоняется — исправьте поле в Prefab Mode. Сохранение данных не означает автоматического преобразования несовместимых полей; восстановление в памяти также не гарантирует полного восстановления вложенного графа.
