...
Автоматическая отрисовка охватывает восемь уровней вложенности, после чего используется стандартная отрисовка Unity. Это ограничение отрисовки, а не запрет на хранение более глубокого графа. Дочернее поле сохраняет свой `[TypeSelector]` или `[CustomPropertyDrawer]`, который Unity применит к нему: для хранимого типа, базового класса, интерфейса или открытого generic-типа вроде `typeof(Effect<>)`.
