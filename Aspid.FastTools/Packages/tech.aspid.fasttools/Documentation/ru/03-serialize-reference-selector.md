...
Если Fix недоступен, выберите один объект и проверьте, что сцена или Prefab Mode сохранены и не имеют несохранённых изменений. Компонент, унаследованный из префаба (вариант, вложенный префаб, экземпляр в сцене), показывает **Missing type** без Fix: почините его в исходном префабе, названном в подсказке. Если тип хранится в override префаба, выберите новый тип или отмените override. Если проблема находится внутри потерянного родителя и поле недоступно, используйте [Asset References](04-serialize-reference-tooling.md#asset-references-разобрать-один-ассет).
