...
| Метод `Run()` в классе `Worker<int>`, свой маркер на каждый закрытый тип | `Worker<Int32>.Run (строка)` |
| Метод `Execute()` в структуре `Job<int>`, один маркер на все закрытые типы | `Job<T>.Execute (строка)` |
| Статический конструктор, на экземпляре типа | `FlockSimulation.StaticCtor (строка)` |

Generic-структура оставляет в имени `<T>`, потому что Burst не может построить имя для каждого закрытого типа.
...
- **Только экземпляр своего типа.** Вызов получает маркер, когда сделан на экземпляре типа, в котором написан: на `this` или на другом экземпляре, например `new FlockSimulation().Marker()` в статическом методе. `other.Marker()` на объекте другого типа и `((Base)this).Marker()` ничего не открывают — или открывают маркер того типа, если номер строки совпал с одним из его вызовов. Анализатор `AFT0010` предупреждает о таких вызовах.
