...
Смена типа подчиняется правилам **Add Component**: она добавляет компоненты из `[RequireComponent]` нового класса и отменяется с предупреждением, если новый класс помечен `[DisallowMultipleComponent]` и такой компонент на GameObject уже есть, или если смена убрала бы класс, который нужен другому компоненту. Один Undo откатывает смену вместе с добавленными компонентами.
