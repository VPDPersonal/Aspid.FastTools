...
> [!WARNING]
> Как только такая строка отрисовывается в инспекторе, её ключ молча заменяется на первый член enum. Переименовывайте члены до открытия ассета в инспекторе или проверьте строки сразу после.
