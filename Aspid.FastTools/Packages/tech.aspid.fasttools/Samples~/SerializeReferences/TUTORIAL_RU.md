...
Каждый урок работает и в инспекторе **UIToolkit**, и в **IMGUI** — пакет поставляет drawer для каждого пути, и они на паритете по функциям. (У каждого демо-префаба есть `IMGUI…`-двойник — через соседние компоненты `IMGUILoadout` / `IMGUISlottedLoadout` — принудительно использующий IMGUI, если хотите сравнить.)
...
> Smart Fix — страховка для переносов, где про атрибут забыли.
...
- **Breakage detection** — включает/выключает проактивный тост о потерянных ссылках; выключите, чтобы полностью
  заглушить детектирование при перезагрузке домена / импорте. Per-machine.
...
Rid colours — не настройка: общая ссылка всегда раскрашивается по id (в полоске/нотисе инспектора и в чипе окна
графа), поскольку в этом и есть смысл функции — совпадающий цвет позволяет с одного взгляда понять, какие поля
делят один экземпляр.

Auto de-alias, build/CI gate и excluded folders должны вести себя одинаково у всей команды и на CI, поэтому, в отличие
от breakage detection (per-machine `EditorPrefs`), они сохраняются в **коммитимый** asset
`ProjectSettings/SerializeReferenceSharedSettings.asset`. Закоммитьте этот asset после изменения любого из этих трёх
значений.

Те же опции доступны и во вкладке **Settings** окна (**`Tools → Aspid 🐍 → FastTools → Settings`**).
Кроме того, вкладка содержит секцию **Type Selector** с индивидуальными настройками пикера — переключатель секции
**Favorites** на корневой странице, слайдер **Recent items** (0–20; 0 скрывает секцию Recent и приостанавливает
запись, не стирая накопленную историю) и строку **Saved lists**, очищающую сохранённые списки Favorites / Recent.
Там же — секция **Appearance** (override-`StyleSheet` темы редактора с действием **Create template…**) и секция
**Welcome** (переключатель **Auto-show Welcome**, глушащий автопоказ Welcome при первом запуске). Поскольку вкладка
смешивает командные и индивидуальные настройки, каждая строка помечена цветной полоской слева — зелёная для значений
в коммитимом asset'е `ProjectSettings`, синяя для индивидуальных — с легендой в верхней части вкладки. Закреплённый
внизу вкладки футер предлагает **Reset to defaults** отдельно для каждого scope (**Shared** / **Per-user**); каждая
кнопка показывает подтверждение с точным списком восстанавливаемых значений, а сохранённые списки Favorites / Recent
сброс переживают. Та же поверхность — секции, легенда и футер сброса — целиком продублирована на странице
**`Preferences → Aspid FastTools`**, так что все поверхности зеркалят друг друга живьём.
...
уровня через чистый YAML-проход), а per-run флаги `-srGateWarnOnly` (форсировать код 0) / `-srGateFail` (форсировать
...
| `Scripts/SlottedLoadout.cs` | ссылки, вложенные в контейнеры (урок 7, отдельно) — его двойник `IMGUISlottedLoadout` форсирует путь IMGUI |
| `Scripts/WeaponPreset.cs` + `Presets/Broken*.asset` / `Presets/MovedWeaponPreset.asset` | сценарий починки потерянного типа — ручной **Fix** и одно-кликовый **Smart Fix** |
| `Scripts/IMGUILoadout.cs` / `Scripts/IMGUISlottedLoadout.cs` (+ их companion'ы в `Editor/`) | те же поля, принудительно через путь IMGUI; у каждого демо-префаба есть `IMGUI…`-двойник |
