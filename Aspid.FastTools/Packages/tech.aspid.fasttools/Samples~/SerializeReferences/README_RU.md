...
- `Scripts/Weapons/` — интерфейс `IWeapon` и его реализации (`Sword`, `Pistol`, `Shotgun`, `Railgun`, `Crossbow`). `Railgun` вкладывает ещё одно поле `[TypeSelector]` — показывает рекурсивное полиморфное редактирование.
...
- `Scripts/WeaponPreset.cs` + `Presets/BrokenWeaponPreset.asset` / `Presets/MovedWeaponPreset.asset` — `ScriptableObject`-ы, у которых поле `_weapon` указывает на идентичность типа, которая больше не резолвится; используются для демонстрации починки потерянного типа и одно-кликового **Smart Fix** (см. *Сервисные функции* ниже).
...
В `Prefabs/` лежат готовые демо-префабы — дважды кликните, чтобы открыть в Prefab Mode, или перетащите любой в сцену. Начните с `Loadout.prefab` / `NestedLoadout.prefab`. Пара **Loadout**:
...
`SlottedLoadout.prefab` — готовое демо урока 7 (ссылки, вложенные в `[Serializable]`-контейнеры). У него и у каждого сценарного префаба из **Сервисных функций** ниже (`LoadoutMissingType`, `NestedLoadout`, `LoadoutSharedRef`) тоже есть `IMGUI…`-двойник — откройте любой рендерер, чтобы увидеть те же данные обоими путями.
...
сохраняя `_damage = 21`, `_magazineSize = 6`. В ранжировании высший балл у объявленного `[MovedFrom]`, дальше —
одноимённый тип в другом namespace/сборке, переименование только по регистру и похожее имя, подкреплённое совпадением
формы полей осиротевших данных; подсказка никогда не применяется автоматически. (Перенос, сразу снабжённый
`[MovedFrom]`, вообще не ломается — Unity мигрирует ссылку при загрузке; Smart Fix ловит переносы, где про атрибут
забыли. У `GhostWeapon`/`GhostPistol`-ассетов выше преемника нет, поэтому подсказки у них не появляется — этот
контраст намеренный.)
