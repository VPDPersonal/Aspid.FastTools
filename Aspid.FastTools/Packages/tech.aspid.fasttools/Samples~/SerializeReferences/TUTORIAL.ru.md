...
Каждый урок работает и в инспекторе **UIToolkit**, и в **IMGUI** — пакет поставляет drawer для каждого пути, и они на паритете по функциям. (`Prefabs/IMGUILoadout.prefab` принудительно использует путь IMGUI, если хотите сравнить — см. *Путь IMGUI* в [README.ru.md](README.ru.md).)
...
> **Важно — `TypeAllow` здесь не применяется.** Опция `Allow` (по умолчанию `TypeAllow.All`) влияет только на
> `[TypeSelector]` для поля-**имени типа** — `string` или `SerializableType`, где вы именуете тип, а не создаёте его;
> там `Allow = TypeAllow.None` ограничивает список только конкретными типами. Для поля `[SerializeReference]` она
> игнорируется — абстрактный класс или интерфейс выбрать нельзя, потому что нечего было бы инстанцировать.
...
| `Scripts/IMGUILoadout.cs` + `Scripts/Editor/IMGUILoadoutEditor.cs` | те же поля, принудительно через путь IMGUI (см. *Путь IMGUI* в [README.ru.md](README.ru.md)) |
