...
5. **Script access.** Double-click the ability name at the top of the detail pane: `AddOpenScriptCommand` opens `AbilityConfig.cs` in your IDE.
6. **The inspector.** Select `Data/Sprint.asset` in the Project window. `AbilityConfigEditor` draws a card titled with `GetDisplayName()` (“Ability Config”), with a status badge and a warning `HelpBox` that appears only while `Mana Cost` is `0`; `PropertyField.AddValueChanged` drives both. Set the cost to `10` and back.
...
| File | Shows |
|---|---|
| `Scripts/Editor/AbilityCatalogWindow.cs` | `ListView` extensions, `BindTo`, `SetFloat` / `SetIntAndApply`, `TypeSelectorWindow.Show` with a `TypeSelectorFilter`, `AddOpenScriptCommand` |
| `Scripts/Editor/AbilityConfigEditor.cs` | A reactive custom inspector with the style and layout setters, `GetDisplayName` |
