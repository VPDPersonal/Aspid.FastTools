...
|---|---|
| **Implementation picking** | The list shows concrete `[Serializable]` non-`UnityEngine.Object` classes compatible with the field type — a type without `[Serializable]` is left out, since Unity would drop the value on the next reload. `[TypeSelector(typeof(IMelee))]` narrows it to `IMelee` implementations, and `[TypeSelectorDisplay(Hidden = true)]` keeps an individual type out of the picker. |
...
| **Nested references** | A `[SerializeReference]` field (or array/list) *inside* the assigned instance gets the same dropdown, so a graph can be authored all the way down without annotating every level. |
