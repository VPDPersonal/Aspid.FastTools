...
internal static class ExtensionClassBody
{
    ...
    private static void GenerateForType(SourceProductionContext context, IGrouping<string, MarkerCall> typeGroup)
    {
        ...
        // Markers are per closed type only when the label varies with the type's own parameters;
        // outer type parameters still reach the Marker<...> signature through TypeParamList.
    ...
    private static string BuildMarkerValueExpression(TypeData type, MarkerCall call)
    {
        ...
        // TypeParamList lists the containing types' parameters first; the label shows only the type's own.
        var typeParamNames = ExtractTypeParamNames(type.TypeParamList).ToArray();
        var typeofExpressions = string.Join(
            " + \", \" + ",
            typeParamNames.Skip(typeParamNames.Length - type.Arity).Select(p => $"typeof({p}).Name"));
