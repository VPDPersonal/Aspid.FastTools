...
- `AFT0007` больше не ломает компиляцию на члене типа `Type?` при `#nullable enable` и теперь сообщает о свойстве без getter, которое инспектор не может прочитать. `AFT0004` теперь сообщает и о поле `UnityEngine.Object?`.
- `AFT0005` больше не предупреждает о generic-типе поля или базовом типе (`IState<Enemy>`, `Base<int>`, `typeof(IFoo<int>)`), у которого есть закрытая или открытая generic-реализация, и о реализации через `out`/`in`-вариантность — она больше не вызывает и `AFT0003`. Ни одно из них не срабатывает на поле типа generic-параметра (`T` в `Slot<T>`). `AFT0003` и `AFT0009` больше не срабатывают на несвязанный `typeof(IFoo<>)` в паре с sealed-типом, который реализует какой-либо `IFoo<X>`.
- `[field: TypeSelector]` и `[field: SerializeReference, TypeSelector]` на автосвойстве теперь проверяются правилами `AFT0001`–`AFT0009`, а `AFT0003` теперь проверяет и поля `SerializableType<T>` и `SerializableMonoScript<T>`.
