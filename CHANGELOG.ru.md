...
- Поле `[SerializeReference]` в варианте префаба, вложенном префабе или экземпляре в сцене с потерянным типом теперь показывает **Missing type** вместо пустого `<None>`; подсказка называет исходный префаб, где нужно чинить тип, или сообщает, что тип хранится в override префаба. С `-srGateRequired` обязательное поле, чей потерянный тип лежит в override варианта, попадает в отчёт как потерянный тип, а не как незаданное.
