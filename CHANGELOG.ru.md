...
- Уведомление о поломке теперь работает с самого начала каждой сессии редактора: раньше оно молчало, пока в этой сессии Project References не просканировал проект. Базовая линия читается из файлов ассетов в фоне и учитывает ассеты, сохранённые позже, поэтому тип, впервые назначенный в этой сессии, тоже отслеживается.
- Удаление скрипта, в котором объявлены только компоненты или ScriptableObject, больше не сканирует все ассеты проекта; для остальных скриптов при долгой проверке появляется прогресс-бар, и кнопка отмены в нём отменяет удаление.
- Защита удаления теперь предупреждает обо всех типах, объявленных в скрипте, в том числе в файле без класса с его именем (`Pistol` и `Rifle` в `Weapons.cs`) и о вложенных типах (`Outer.Inner`); раньше проверялся только класс с именем файла.
