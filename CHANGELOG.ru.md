...
- **Fix** и **Smart Fix** отсутствующего типа в сохранённом ассете в инспекторе UI Toolkit больше не выводят исключение об освобождённом `SerializedObject`; остальные поля ссылок теперь обновляются сразу после ремонта.
