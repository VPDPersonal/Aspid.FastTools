...
- Инспектор `EnumValues` больше не переписывает ключи при отрисовке: строка, ключ которой enum не может распознать (после смены enum в `EnumValues<TValue>`, переименования или удаления члена), показывается как `<Missing Имя>` и сохраняет ключ, пока вы не выберете член. Раньше ей подставлялся первый член enum, и возврат прежнего типа строки не восстанавливал.
