...
- Поиск в селекторе типов больше не совпадает с частью имени, где указана сборка: короткие запросы вроде `Key`, `Token`, `ver` или `null` находили все типы. Поиск сравнивает подпись, имя типа и `Namespace.Name`.
