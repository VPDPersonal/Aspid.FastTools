...
- Удаление скрипта, в котором объявлены только компоненты или ScriptableObject, больше не сканирует все ассеты проекта; для остальных скриптов при долгой проверке появляется прогресс-бар с отменой.
