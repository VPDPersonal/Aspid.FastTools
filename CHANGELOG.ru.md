...
- В редакторе `SerializableMonoScript` теперь берёт тип из своего скрипта, если сохранённое имя больше не находится. Объект, загруженный в Play Mode после переименования класса (дополнительная сцена, `Resources.Load`, вход в Play Mode без перезагрузки домена и сцены), больше не получает `null`, пока его ассет не пересохранён.
- `SerializableType` и `SerializableMonoScript` теперь кешируют и неудачный поиск: чтение `.Type` у потерянного типа больше не вызывает `Type.GetType` каждый раз, а кеш сбрасывается при смене сохранённого имени.
