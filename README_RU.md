...
- Потерянный тип можно починить на месте: в предупреждении есть кнопка **Fix**, открывающая селектор типов — выбранный тип переписывается прямо в YAML ассета с сохранением данных. Работает для сохранённых ассетов (ScriptableObject и префаб-ассетов), выделенных в Project; осиротевшие тип и данные восстанавливаются прямо из файла, даже когда Unity обнулил живую ссылку.
- До потерянных ссылок, недоступных в инспекторе (вложенные значения, компоненты дочерних объектов), есть окно **Repair Missing References** (`Tools → Aspid FastTools`): оно сканирует весь файл ассета и выводит каждую со своим **Fix** — без Prefab Mode.
- Общая ссылка (два поля делят один экземпляр, например после дублирования элемента списка) помечается, а контекстное действие **Make Unique Reference** расщепляет её в независимую копию.
