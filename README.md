...
- The **Managed References** window (`Tools → Aspid 🐍 → Managed References FastTools`) maps an asset's whole managed-reference graph from the YAML: a per-component tree of field-pointer roots, nested children (each labelled with its full field path, e.g. `_primaryWeapon._chargeEffect`), shared references and orphaned payloads, with `MISSING` / `SHARED` badges, deterministic per-rid colours, a constrained inline **Fix** for missing entries (whose `<None>` option clears the reference), and a **Clear** action on any assigned reference that resets it to `<None>`. It surfaces references at any nesting depth — including unassigned `[SerializeReference]` fields, shown as dim `<None>` slots so a cleared or never-set reference stays visible — and the orphans the Inspector cannot navigate to.
