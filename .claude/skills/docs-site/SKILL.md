...
| Tutorials | `Samples~/<Sample>/README.md` and `TUTORIAL.md` | `/tutorials/<slug>`, `/tutorials/<slug>/tutorial` (`SerializeReferences` → `serialize-references`; an optional `NN. ` folder prefix orders and is stripped) |
...
- **Images** live in `Documentation/Images/` and are referenced relatively (`Images/x.png` from a doc,
  `../Images/x.png` from `ru/`, `../../Documentation/Images/x.png` from a sample).
...
- The package is English. A translation is a sibling file: `Documentation/ru/06-enum-values.md`,
  ...
  targets (`../../Samples~/Types/README.ru.md`) so GitHub stays in the same language; the site drops the
...
1. Create `Samples~/<Name>/README.md` (+ `TUTORIAL.md` if there is a guided scene), with `.meta` files.
