...
- Added the `[TypeSelectorItem]` attribute (`Aspid.FastTools.Types`, editor-only `[Conditional("UNITY_EDITOR")]`) for tuning how a type appears in the picker: a custom display name or a `"Category/Name"` path that re-homes the type under category nodes (a plain name renames the leaf in place), plus `Tooltip`, `Order` (lower sorts higher, ties broken alphabetically) and `Icon` (an `EditorGUIUtility.IconContent` name or a `Resources` texture path). Search still matches the real type name.
- The type picker's root page now shows **Favorites** and **Recent** sections. A hover-revealed ★ toggle on each row pins a type to Favorites; the last 8 picked types are kept in MRU order under Recent. Both are persisted per project in `EditorPrefs`, pruned of types that no longer resolve, surface only types in the current candidate set, and are hidden while searching.
...
- Fields that share one managed reference now get a deterministic per-rid colour stripe and chip in the Inspector notice, matching the colours used by the **Managed References** window so the same shared instance reads the same colour across both surfaces.
...
- Added a `Required` flag on `[TypeSelector]` (`Aspid.FastTools.Types`) for both field shapes: an unset required `[SerializeReference]` managed reference (empty == null) or `string` type field (empty == no type) shows an inline warning in the inspector (IMGUI and UIToolkit) and counts as a violation for the build/CI gate. An optional `RequiredMessage` overrides the notice text.
- Added a **Project Settings** page (`Project Settings → Aspid FastTools → SerializeReference`) to toggle rid colours, auto de-alias, breakage detection, the build/CI gate severity, and excluded scan folders.
- Added a **build / CI gate**: an `IPreprocessBuildWithReport` that warns or fails a player build on missing managed references (Project Settings severity, Warn by default), and a headless `-executeMethod Aspid.FastTools.SerializeReferences.Editors.SerializeReferenceCiGate.RunCheck` entry point that scans the project, writes a report, and exits non-zero for CI (`-srGateReport`, `-srGateRequired`, `-srGateWarnOnly` flags).
- Added a **Project Settings** page (`Project Settings → Aspid FastTools → SerializeReference`) to toggle rid colours, auto de-alias, the build/CI gate severity, and excluded scan folders.
- Added a **build / CI gate**: an `IPreprocessBuildWithReport` that warns or fails a player build on missing managed references (Project Settings severity, Warn by default), and a headless `-executeMethod Aspid.FastTools.SerializeReferences.Editors.SerializeReferenceCiGate.RunCheck` entry point that scans the project, writes a report, and exits non-zero for CI (`-srGateReport`, `-srGateRequired`, `-srGateWarnOnly` flags). The `-srGateRequired` check covers prefabs, ScriptableObjects **and scenes** — a `.unity` is read through a pure-YAML pass (resolving each MonoBehaviour's required fields by its `m_Script` guid) for top-level required fields, since scene objects cannot be loaded for inspection.
