...
- Undo after **Fix** of a missing type in an open scene or Prefab Mode brings back the missing reference with its stored data; it used to leave the field empty, and the next save wrote `rid: -2`. The replaced entry is now dropped when the scene or prefab is saved, and from then on the repair cannot be undone.
