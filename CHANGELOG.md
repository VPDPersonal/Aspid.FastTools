...
- The `EnumValues` Inspector no longer rewrites keys while drawing: a row whose key the enum cannot parse (after switching `EnumValues<TValue>` to another enum, or renaming or deleting a member) shows `<Missing Name>` and keeps its key until you pick a member. It used to get the enum's first member, so switching the type back did not bring the rows back. A row added to an empty table now shows `<None>` until you pick a member.
...
- An `EnumValues` row of a `[Flags]` enum based on `long` or `ulong` gets a flags dropdown that keeps all 64 bits; the IMGUI Inspector used to throw on every repaint, and the UI Toolkit one cut the mask and stored a different key.
