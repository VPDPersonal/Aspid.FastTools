...
- The **Settings** tab also gained a **Welcome** section with an **Auto-show Welcome** toggle that suppresses the auto-open of the Welcome tab (the menu entry keeps working), covered by the per-user **Reset to defaults** (auto-show → on).
- The **Preferences → Aspid FastTools** page (previously theme-only) is now a full mirror of the window's Settings tab — the same References / Type Selector / Welcome sections, scope legend and per-scope **Reset to defaults** footer, rendered on the window's branded surface (cards, scope stripes and its own dotted canvas in the Settings tab's idle tone). The scope stripes — not the page — say what is committed and what is local; the SerializeReference Project Settings page still exposes the References controls in Unity's native look. Every surface is built from one control definition per area and mirrors the others live.
