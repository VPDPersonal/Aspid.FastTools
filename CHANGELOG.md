...
- The `EnumValues` Inspector no longer rewrites keys while drawing: a row whose key the enum cannot parse (after switching `EnumValues<TValue>` to another enum, or renaming or deleting a member) shows `<Missing Name>` and keeps its key until you pick a member. It used to get the enum's first member, so switching the type back did not bring the rows back.
