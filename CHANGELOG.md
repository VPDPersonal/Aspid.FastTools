...
- **The samples were rebuilt around small working scenes instead of step-by-step tutorials.** Each sample is now one scene (or, for EditorTools, one window) that does something visible with the feature, and one `README.md` that says what to try and where to look; the `TUTORIAL.md` files and the `STEP 1…N` tutorial components are gone, since they repeated the main documentation. Types spawns enemies through a rename-safe `SerializableMonoScript<T>`, a `SerializableType<T>` strategy with `[TypeSelectorDisplay]` candidates, a member-referenced `[TypeSelector]` and `ComponentTypeSelector`; SerializeReferences fires polymorphic weapons at a dummy and keeps its broken presets and a broken prefab for the repair tools; EnumValues walks a character over surface tiles; Ids is an item catalog with a typed `IdRegistry<T>` and an int-keyed inventory; ProfilerMarkers is a flock simulation; and **VisualElements became EditorTools**, an editor window that also covers `SerializedProperty` setters, `GetScriptName`, `AddOpenScriptCommand` and `TypeSelectorWindow` as a public API.
...
- **The ID System moved to its own package, [Aspid.Ids](https://github.com/VPDPersonal/Aspid.Ids) (`tech.aspid.ids`).** `IId`, `IdRegistry`, `IdRegistry<T>`, `[UniqueId]`, the registry inspector, the id dropdown, the `IdStructGenerator` and the `Ids` sample are no longer part of FastTools. The feature was a data system with its own assets, conventions and roadmap rather than a tool, and nothing else in the package depended on it. Install `tech.aspid.ids` next to FastTools and replace `using Aspid.FastTools.Ids;` with `using Aspid.Ids;` — registry assets and serialized id fields keep their data. Numbering of the remaining documentation pages shifted down by one (`07-visual-element-extensions.md` … `10-claude-code-plugin.md`).
...
- **Breaking: the package layout is now the conventional `Runtime/` and `Editor/`, and the assemblies lost their `.Unity` segment.** `Source/` and `Unity/` are gone: `Unity/Runtime/` and the two engine-free extension files from `Source/` became `Runtime/Scripts/`, `Unity/Editor/Scripts/` became `Editor/Scripts/` and `Unity/Editor/Resources/` became `Editor/Resources/`. With the `Unity` folder gone the suffix no longer described anything, so `Aspid.FastTools.Unity` is now `Aspid.FastTools`, `Aspid.FastTools.Unity.Editor` is `Aspid.FastTools.Editor`, and the satellite and test assemblies follow (`Aspid.FastTools.VisualElements.Math`, `Aspid.FastTools.Editor.SerializeReferences.Yaml`, `Aspid.FastTools.Editor.Tests`, `Aspid.FastTools.Editor.SerializeReferences.Tests`). The separate engine-free `Aspid.FastTools` assembly was folded into the runtime one — its `StringExtensions` and `TypeExtensions` now live in `Runtime/Scripts/Extensions/` and keep their `Aspid.FastTools` namespace, so nothing changes for calling code. Namespaces never carried `.Unity` and are untouched; an `asmdef` that referenced `Aspid.FastTools.Unity` or `Aspid.FastTools.Unity.Editor` has to be updated to the new names.
