...
- Added a **Project Settings** page (`Project Settings → Aspid FastTools → SerializeReference`) to toggle rid colours, auto de-alias, breakage detection, the build/CI gate severity, and excluded scan folders. Rid colours and breakage detection are per-machine `EditorPrefs`; auto de-alias, the build/CI gate severity and excluded scan folders must behave the same for every teammate and for CI, so they are stored in a committed `ProjectSettings/SerializeReferenceSharedSettings.asset` instead.
