...
- `AFT0005` no longer warns about a generic field or base type (`IState<Enemy>`, `Base<int>`, `typeof(IFoo<int>)`) that has a closed or open generic implementation, or about an implementation through `out`/`in` variance, which also no longer triggers `AFT0003`. Neither warns about a field of a type parameter (`T` in `Slot<T>`). `AFT0003` and `AFT0009` no longer report an unbound `typeof(IFoo<>)` paired with a sealed type that implements some `IFoo<X>`.
