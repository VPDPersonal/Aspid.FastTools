...
- Added the `[SerializeReferenceRequired]` attribute (`Aspid.FastTools.Types`, editor-only `[Conditional("UNITY_EDITOR")]`): an unset required reference shows an inline notice in the inspector (IMGUI and UIToolkit) and counts as a violation for the build/CI gate.
