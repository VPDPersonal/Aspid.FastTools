...
- `[SerializeReferenceSelector]` can now repair a missing managed-reference type: the missing-type warning offers a **Fix** button that opens the hierarchical type picker, and the chosen type is rewritten directly in the asset YAML. Detection and repair read the orphaned type and data straight from the asset file, so they work even though Unity neither exposes a missing type through the serialization API nor keeps it on the live object for prefabs/GameObjects (UUM-129100). Re-pointing the type keeps the stored data instead of recreating the instance. Applies to saved assets (ScriptableObjects and prefab assets) selected in the Project — not to scene objects or Prefab Mode editing, which have no backing asset file to rewrite.
...
- Added a **Repair Missing References** window (`Tools → Aspid FastTools → Repair Missing References`) that scans a selected prefab/ScriptableObject's YAML and lists *every* orphaned managed reference — at any nesting depth and on any child object — each with a **Fix** picker. This reaches missing references the per-field drawer cannot (nested values, child-object components, anything Unity dropped to `<None>`) and never requires Prefab Mode.
