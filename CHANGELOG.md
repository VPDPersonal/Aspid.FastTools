...
- `TypeSelectorWindow.Show` gained an optional `TypeSelectorFilter` parameter — its `Predicate` further narrows the candidate list after the base-type and `TypeAllow` checks (used by the SerializeReference drawer to exclude `UnityEngine.Object`, strings and delegates), and its `AdditionalTypes` injects entries the assignability scan cannot match (such as open generic definitions).
...
- `[TypeSelector]` can now repair a missing managed-reference type. The warning is a compact yellow notice whose underlined **Fix** word opens the hierarchical type picker; the chosen type re-points the reference while keeping its stored data, and hovering the notice shows the full missing-type detail. The broken value itself reads as part of the warning too: the dropdown caption is tinted the same warning amber as the stripe and the notice, truncates from the **left** so the class name — the informative tail of `<Missing Namespace.Class>` — stays visible, and hovering the dropdown shows the full stored identity, assembly included. Detection reads the orphaned type straight from the asset YAML, so it works even though Unity neither exposes a missing type through the serialization API nor keeps it on the live object for prefabs/GameObjects (UUM-129100). Repair applies to saved assets (ScriptableObjects and prefab assets) selected in the Project **and to objects open in Prefab Mode**: saved assets are rewritten in their YAML, while a Prefab Mode object is repaired on the live instance — reassigning the reference and recovering the field data Unity still holds for the missing type — because its open stage would discard a file rewrite on save. The repair resolves the reference at any depth — through nested managed references and through plain `[Serializable]` containers (a struct/class field or a `List<T>` of them), so a missing type buried in a slot or list element is fixed inline too; and, for a Prefab Mode object, when the repaired reference itself carried missing nested references, those now-orphaned entries are cleared as well so the object's missing-types banner clears (the saved-asset YAML path only rewrites the `type:` line and leaves nested data untouched). Works in both IMGUI and UIToolkit paths.
...
- Added the `[TypeSelectorDisplay]` attribute (`Aspid.FastTools.Types`, editor-only `[Conditional("UNITY_EDITOR")]`) for tuning how a type appears in the picker: a `Name` display override for the picker rows and the closed dropdown's caption (search still matches the real type name, and the hover tooltip keeps revealing the real `Namespace.Class, Assembly` identity), a `Group` path (e.g. `"Combat/Melee"`) that places the type under an explicit picker hierarchy **instead of** its namespace (path segments are shared between types, so related types meet under one node), a `Tooltip` shown on the type's row and an `Icon` (an `EditorGUIUtility.IconContent` name, a project-relative asset path with extension loaded via `AssetDatabase`, or a `Resources` texture path without extension).
- The type picker's root page now shows **Favorites** and **Recent** sections. A hover-revealed ★ toggle on each row pins a type to Favorites; the last picked types (5 by default, configurable) are kept in MRU order under Recent. Both are persisted per project in `EditorPrefs`, pruned of types that no longer resolve, surface only types in the current candidate set, and are hidden while searching.
...
- The window's **Settings** tab gained a **Type Selector** section with the picker's per-user preferences: a toggle that hides the root page's **Favorites** section (the stored list survives and comes back when re-enabled), a **Recent items** capacity slider (0–20, default 5) whose 0 doubles as the Recent section's off switch — it hides the section and pauses recording without wiping the collected history — and a **Saved lists** maintenance row that clears the stored Favorites / Recent lists behind a count-naming confirmation. And since the tab now mixes team-wide and individual settings, every row is marked with a storage-scope stripe — green for settings in the committed `ProjectSettings` asset, blue for per-user `EditorPrefs` ones — decoded by a compact legend at the top of the tab, and a footer pinned under the scroll offers **Reset to defaults** separately per scope (**Shared** / **Per-user**), each behind a confirmation naming the exact defaults it restores (the saved Favorites / Recent lists are data, not settings, and survive a reset).
- The **Settings** tab also gained an **Appearance** section — the editor-theme override `StyleSheet` (layered over the built-in Default-Dark palette, applied live), previously reachable only on its own Preferences page, plus a **Create template…** action that writes a commented starter override and assigns it — and a **Welcome** section with an **Auto-show Welcome** toggle that suppresses the auto-open of the Welcome tab (the menu entry keeps working). The per-user **Reset to defaults** covers both (theme override → none, auto-show → on).
- The **Preferences → Aspid FastTools** page (previously theme-only) is now a full mirror of the window's Settings tab — the same References / Type Selector / Appearance / Welcome sections, scope legend and per-scope **Reset to defaults** footer, rendered on the window's branded surface (cards, scope stripes and its own dotted canvas in the Settings tab's idle tone). The scope stripes — not the page — say what is committed and what is local; the SerializeReference Project Settings page still exposes the References controls in Unity's native look. Every surface is built from one control definition per area and mirrors the others live.
...
- Fields that share one managed reference now get a deterministic colour stripe, chip and notice text — assigned per shared-reference group index within the inspected object, so every field in the same group reads the same colour (the **Managed References** window colours its own per-rid chips independently, so the two surfaces don't necessarily agree on a colour for the same instance). Always on — there is nothing to opt out of. The notice's badge numbers the group as `Shared reference #1`, `#2`, … (`#` so the number reads as a group id, never a member count), its hover tooltip lists the other fields using the instance by their inspector paths (`Sidearms › Element 1`), and clicking the message reveals them — the inspector scrolls to the group's next member and briefly pulses every member in the group colour. Works in both IMGUI and UIToolkit paths.
...
- `[MovedFrom]` renames are now told apart from real breakages. A stored type that no longer loads but is claimed by exactly one type's declared `[MovedFrom]` (an ambiguous claim resolves to nothing) is not actually broken — Unity migrates the reference in memory at load; only the YAML on disk keeps the old name. The breakage toast/console message now reads "carries an outdated type name after a [MovedFrom] rename — open Repair to migrate" (a mixed report keeps the missing alarm but names how many entries are auto-migratable), and the **Project References** group card for such a type renders as a calm info-tinted pending migration: an authoritative one-click **Migrate all (N) → NewType** button replaces the heuristic Smart Fix suggestion, running the same confirm + diff preview + Undo flow as a picked fix. The **Asset References** graph mirrors the classification: a pending-migration card wears the info pill instead of the amber one, its band reads a neutral `Fix ▼`, a one-click **Migrate → Type** row bakes the rename for that entry, and the overview headline counts pending migrations separately from missing references (a graph whose only findings are migrations reads info-blue, not amber). A genuinely missing card surfaces the same one-click **Smart Fix** suggestion (`→ Name?`) as the inline notice and the Project References group, so the heuristic quick-apply is available on every missing-type surface. The build/CI gate agrees: a pending migration is not a violation, so a properly declared rename can never warn or fail a build or a CI run — only genuinely unresolvable stored types count. Baking the rename into the files is what lets the `[MovedFrom]` attribute be deleted once no file stores the old name.
- A new installable **SerializeReferences** sample (imported via Package Manager) demonstrating the `[TypeSelector]` managed-reference picker across single fields, `List<T>`, abstract bases, narrowing, nested references, generics and `Required`, in both UIToolkit and IMGUI inspectors, with a step-by-step `TUTORIAL` and a guided `TypeSelectorTutorial` scene. The sample ships pre-broken repair assets covering the recovery paths: `Ghost*` types with no plausible successor exercise the manual **Fix** picker (inline, whole-asset and project-wide sweep), `MovedWeaponPreset.asset` — storing `Pistol` under an old namespace, as after a move without `[MovedFrom]` — surfaces the one-click **Smart Fix** suggestion (`→ Pistol?`), and `RenamedWeaponPreset.asset` — storing the old class name of the `[MovedFrom]`-renamed `Crossbow` — surfaces the Project References **Migrate all** migration flow.

### Changed
- The Welcome tab auto-opens once per package version instead of once per project — after installing **or updating** the package, the next editor launch shows the Welcome tab again (the **Auto-show Welcome** toggle still suppresses every auto-open; the menu entry is unaffected).
...
- The UIToolkit and IMGUI `[TypeSelector]` managed-reference drawers no longer disagree on notice placement. All UIToolkit notices now live in a dedicated host inserted **after** the foldout content: the missing / required / mixed-values notices render only when the field holds no value (so there are no child fields to sit under and their position reads the same in both renderers), while the shared-reference notice deliberately sits at the very bottom, under the assigned instance's nested properties, in **both** renderers.
- **Make unique**, the silent list de-alias and a keep-data type switch no longer drop nested `[SerializeReference]` children. A type switch now carries the shared children over by reference (the old parent is discarded, so reuse is what preserves them), and the unique-making flows deep-copy the whole reference graph — internal aliasing topology preserved, cyclic graphs safe.
...
- Assorted correctness: `SerializableType.Type` no longer throws for a code-constructed instance (null stored name); the required string-field notice no longer captures a disposable `SerializedProperty` in its deferred callbacks; Recent picks record a constructed generic's open definition (the closed form could never surface and silently evicted real entries); Prefab Mode repairs bail when the stage is dirty (the sibling-index replay could land on the wrong asset object); the duplicate guard only fires on exact single-element growth, so a bulk restore (Paste Component Values, Revert) no longer silently de-aliases intentional sharing; settings mirrors survive dock moves and a clicked switch (the focus guard now skips only in-progress text edits); the theme override is stored per project; `AspidSwitch` neutrals flip with the editor skin so the handle stays visible on the light theme; and the `<None>` row no longer wears the current-value ✓ in pickers that have no current value at all (a list `+` append, a missing-type Fix, the bulk project picker).
