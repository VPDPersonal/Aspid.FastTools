...
- A `[SerializeReference]` field of a prefab variant, nested prefab or scene instance whose type is missing now shows **Missing type** instead of an empty `<None>`; the tooltip names the source prefab to repair, or says the type is stored in a prefab override. With `-srGateRequired`, a required field whose missing type sits in a variant override is reported as a missing type, not as unset.
