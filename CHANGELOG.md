...
- `AFT0005` no longer warns about a generic field or base type (`IState<Enemy>`, `Base<int>`, `typeof(IFoo<int>)`) that has a closed or open generic implementation, or about an implementation through `out`/`in` variance, which also no longer triggers `AFT0003`. Neither warns about a field of a type parameter (`T` in `Slot<T>`).
