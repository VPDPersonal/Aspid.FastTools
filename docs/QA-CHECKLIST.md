...
- [ ] **EnumValues sample**: imports via Package Manager without errors; the `EnumValuesTutorial` scene opens and the TUTORIAL steps are walkable (type picker, Populate Missing Enum Members, `[Flags]` lookup rules, `Log Tutorial Lookups`, the typed `EnumValues<TEnum, TValue>` step without a type-picker row); the `EnumValues` demo scene prints the expected line on Space; the `IMGUIDamageDealer` GameObject renders the same fields correctly with a forced-IMGUI inspector.
