...
- [ ] `TypeSelectorDisplay(Hidden = true)`: типа нет в пикере ни на одном пути — обычный скан, поиск, внедрённое открытое generic-определение, пикер managed-ссылки; наследник скрытого типа по-прежнему предлагается, а уже сохранённое значение скрытого типа продолжает отрисовываться. Применим к интерфейсу наравне с классом/структурой и сохраняется у типа, собранного вне Unity (прекомпилированная `.dll`).
...
- [ ] Вложенная ссылка на prefab-инстансе показывает признак переопределения и предлагает Revert / Apply в контекстном меню.
