...
- [ ] `TypeSelectorDisplay(Hidden = true)`: типа нет в пикере ни на одном пути — обычный скан, поиск, внедрённое открытое generic-определение, пикер managed-ссылки; наследник скрытого типа по-прежнему предлагается, а уже сохранённое значение скрытого типа продолжает отрисовываться. Применим к интерфейсу наравне с классом/структурой.
- [ ] `Hidden` и починка: пикер **Fix** для потерянной ссылки и групповая починка в окне References скрытые типы **предлагают** (данные с таким типом должны оставаться перенаправляемыми), а **Smart Fix** их не подсказывает никогда.
...
- [ ] У типа без сериализуемых полей стрелка разворота не рисуется; тип с полями по-прежнему разворачивается.
- [ ] Вложенные managed-ссылки внутри назначенного экземпляра тоже получают дропдаун, включая массив/список таких (`+` открывает пикер, добавленный элемент заполняется); вложенное поле с `[TypeSelector]` сохраняет своё сужение.
- [ ] Вложенное поле, для которого у Unity есть drawer, его сохраняет: зарегистрированный на тип поля `[CustomPropertyDrawer]` по-прежнему рисует строку, а `[Header]` / `[Space]` / `[Tooltip]` на вложенной ссылке отображаются **без** потери дропдауна.
- [ ] Дети появляются при первом разворачивании (свёрнутая ссылка не строит ничего); глубокая цепочка перестаёт вкладываться на 8 уровнях и переходит на отрисовку Unity, а не виснет и не падает.
- [ ] Кнопка открытия скрипта открывает вложенный тип (объявленный внутри другого) на строке его объявления, а не только типы верхнего уровня; вложенный тип, объявленный в другом файле `partial`-типа, сообщает «не найдено», а не открывает чужой файл.
- [ ] `ComponentTypeSelector`: выбор вложенного наследника либо корректно меняет компонент, либо пишет «MonoScript not found» — но никогда не подставляет молча скрипт объявляющего типа.
...
- [ ] Кандидат, чей параметр не попадает ни в одно поле по значению, закрывается аргументом, который Unity сериализовать не умеет: поле `IConverter<Ray, Ray>` показывает `SequenceConverters<Ray>`, а кандидат, хранящий `T`, его по-прежнему отвергает.
