...
- [ ] **Сэмпл EnumValues**: импорт через Package Manager без ошибок; сцена `EnumValuesTutorial` открывается, TUTORIAL-шаги проходимы (пикер типа, Populate Missing Enum Members, правила `[Flags]`-поиска, `Log Tutorial Lookups`, типизированный шаг `EnumValues<TEnum, TValue>` без строки выбора типа); демо-сцена `EnumValues` печатает ожидаемую строку по Space; GameObject `IMGUIDamageDealer` корректно рендерит те же поля с принудительно IMGUI-инспектором.
