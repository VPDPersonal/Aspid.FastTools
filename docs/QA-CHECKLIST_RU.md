...
- [ ] `TypeSelectorDisplay(Hidden = true)`: типа нет в пикере ни на одном пути — обычный скан, поиск, внедрённое открытое generic-определение, пикер managed-ссылки; наследник скрытого типа по-прежнему предлагается, а уже сохранённое значение скрытого типа продолжает отрисовываться.
...
- [ ] Предлагаются только `[Serializable]`-типы — кандидат без атрибута (который Unity сбросил бы при перезагрузке) в пикере не появляется.
...
- [ ] Кнопка открытия скрипта открывает вложенный тип (объявленный внутри другого) на строке его объявления, а не только типы верхнего уровня.
