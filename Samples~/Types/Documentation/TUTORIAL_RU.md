...
- `SerializableType<T>` показывает кандидатов с `TypeAllow.All`, поэтому в списке есть и сам тип `T` — **в том
  ...
  **конкретный** наследник, ведь абстрактный тип инстанцировать нельзя. Сравните с уроком 2: `[TypeSelector]` по
  умолчанию использует `TypeAllow.None` и скрывает абстрактные базы и интерфейсы.
...
- `Allow` по умолчанию `TypeAllow.None`: абстрактные базы и интерфейсы скрыты. Укажите
  `[TypeSelector(typeof(AbilityModifier), Allow = TypeAllow.Abstract)]` (или `Interface` / `All`), чтобы их включить.
