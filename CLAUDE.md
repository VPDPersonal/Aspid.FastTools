...
| Представление Welcome | Не отдельное окно — вкладка `TabWindow` (`Windows/Welcome/`), плюс `WelcomeWindowStartup` (автопоказ при первом импорте); перечисляет устанавливаемые примеры из `package.json` |
